...
function setInitialTables()
	...
	sgs.wizard_skill = 		"guicai|guidao|jilve|tiandu|luoying|noszhenlie|huanshi|luawangcai"
	sgs.wizard_harm_skill = 	"guicai|guidao|jilve|luawangcai"
	...
	sgs.save_skill = 		"jijiu|buyi|nosjiefan|chunlao|longhun|longhunluavs"
	...
	sgs.need_equip_skill = 		"shensu|mingce|jujian|beige|yuanhu|gongqi|nosgongqi|yanzheng|qingcheng|neoluoyi|longhun|shuijian|supershuijian|longhunluavs"
...
function SmartAI:getCardNameBySuitString(suit_string)
	...
	if suit_string == "heart" then
		return "peach"
	elseif suit_string == "spade" then
		return "nullification"
	elseif suit_string == "club" then
		return "jink"
	elseif suit_string == "diamond" then
		return "fire_slash"
	end
end

function SmartAI:getLongHunUseValue(card)
	...
	local use_value = self:getUseValue(card)                     -- sgs.ai_use_value[class_name]
	local implicit_use_value = self:getUseValue(implicit_card)   -- sgs.ai_use_value[implicit_card_name]
...
function SmartAI:getLongHunKeepValue(card)
	local card_name   = card:objectName()
	local suit_string = card:getSuitString()
	...
	local use_value = self:getKeepValue(card)                     -- sgs.ai_use_value[class_name]
	local implicit_use_value = self:getKeepValue(implicit_card)   -- sgs.ai_use_value[implicit_card_name]
...
function SmartAI:sortByLongHunUseValue(cards , inverse)
	local compare_func = function(a,b)
		local value1 = self:getLongHunUseValue(a)
		local value2 = self:getLongHunUseValue(b)
...
function SmartAI:sortByLongHunKeepValue(cards , inverse)
	local compare_func = function(a,b)
		local value1 = self:getLongHunKeepValue(a)
		local value2 = self:getLongHunKeepValue(b)
...
function SmartAI:getLongHunRetrialCardId(cards, judge)
	local can_use = {}
	...
	if next(can_use) then
		self:sortByLongHunUseValue(can_use,true)
		...
		elseif data:toString() then
			...
			if callbacktable and type(callbacktable) == "table" then
				...
				if promptlist[1] == "cardResponded" then
					...
					if (promptlist[3] == "@guicai-card" or promptlist[3] == "@guidao-card" or promptlist[3] == "@luawangcai_card" )
						and promptlist[#promptlist] ~= "_nil_" then
...
function SmartAI:canRetrial(player, to_retrial)
	...
	elseif player:hasSkill("luawangcai") then
		return player:getHandcardNum() > 0 
	end
end
